...
WHERE ubg."addedToCollectionAt" > ? and ubg."removedFromCollectionAt" is null and ubg."userId" = ? ');
    ...
    $addedBoardGames = (array) $query->fetchAll(PDO::FETCH_ASSOC);
...
WHERE ubg."removedFromCollectionAt" > ? and ubg."userId" = ? ');
    ...
    $query->execute([$syncData["lastSync"], $userId]);
    $removedBoardGames = $query->fetchAll(PDO::FETCH_ASSOC);
    ...
    $query->execute([$syncData["lastSync"], $userId]);
    $addedPlays = $query->fetchAll(PDO::FETCH_ASSOC);
    ...
    $query->execute([$syncData["lastSync"], $userId]);
    $deletedPlays = $query->fetchAll(PDO::FETCH_ASSOC);
    ...
    if (count($boardGameIds) > 0) {
      ...
      $responseSyncData["plays"]["boardGames"] = $query->fetchAll(PDO::FETCH_ASSOC);
    }
...
function getDiffByIds($dbItems, $clientItems): array
{
  ...
  $missingInDb = array_values(array_filter($clientItems, fn($item) => !in_array($item["id"], $dbIds)));

  $missingInClient = array_values(array_filter($dbItems, fn($item) => !in_array($item["id"], $clientIds)));
